...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        if (pair.isPresent()){
            ...
            rootKeys.add(pair.get().getFirst());
            rootChildren.add(root.getPage().getPageNum());
            rootChildren.add(pair.get().getSecond());
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator() {
            this.curLeafNode = root.getLeftmostLeaf();
            this.iterator = curLeafNode.scanAll();
        }

        public BPlusTreeIterator(DataBox key) {
            this.curLeafNode = root.get(key);
            this.iterator = curLeafNode.scanGreaterEqual(key);
        }

        @Override
        public boolean hasNext() {
            // TODO(proj2): implement
            if (iterator.hasNext() || curLeafNode.getRightSibling().isPresent()){
                return true;
            }
            return false;
        }
        ...
        public RecordId next() {
            ...
            else if (curLeafNode.getRightSibling().isPresent()){
                this.curLeafNode = curLeafNode.getRightSibling().get();
                this.iterator = curLeafNode.scanAll();
                if (iterator.hasNext()){
                    return iterator.next();
                }
