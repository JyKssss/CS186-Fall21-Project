...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.contains(key)){
            throw new BPlusTreeException("There exists a duplicate key in leaf");
        }

        int limit = metadata.getOrder(), insertIndex = InnerNode.numLessThanEqual(key, keys);

        keys.add(insertIndex, key);
        rids.add(insertIndex, rid);

        if (keys.size() <= limit*2){
            sync();
            return Optional.empty();
        }

        List<DataBox> rightKeys = new ArrayList<>();
        List<RecordId> rightRecordIds = new ArrayList<>();
        for (int i = 0; i < limit + 1; i++) {
            rightKeys.add(0, keys.get(keys.size()-1));
            rightRecordIds.add(0, rids.get(rids.size()-1));
            keys.remove(keys.size()-1);
            rids.remove(rids.size()-1);
        }

        LeafNode rightNewNode = new LeafNode(metadata, bufferManager, rightKeys, rightRecordIds, this.rightSibling, treeContext);
        Long rightPageNum = rightNewNode.page.getPageNum();
        this.rightSibling = Optional.of(rightPageNum);
        Optional<Pair<DataBox, Long>> pair = Optional.of(new Pair<>(rightKeys.get(0), rightPageNum));
        sync();
        return pair;
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext() && keys.size() <= limit){
            ...
            this.keys.add(record.getFirst());
            this.rids.add(record.getSecond());
        }
        ...
        if (keys.size()>limit){
            ...
            LeafNode rightSibNode = new LeafNode(metadata, bufferManager, rightKeys, rightRids, this.rightSibling, treeContext);
            Long rightPageNum = rightSibNode.page.getPageNum();
            this.rightSibling = Optional.of(rightPageNum);
            pair = Optional.of(new Pair<>(rightKeys.get(0), rightPageNum));
        }
        sync();
        return pair;
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        if (right == -1L) {
            rightSibling = Optional.ofNullable(null);
        } else {
