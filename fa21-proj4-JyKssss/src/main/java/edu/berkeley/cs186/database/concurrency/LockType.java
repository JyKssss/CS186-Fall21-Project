...
public enum LockType {
    ...
    public static boolean compatible(LockType a, LockType b) {
        ...
        int indexA = lockIndexHelper(a), indexB = lockIndexHelper(b);
        if (indexA == -1 || indexB == -1){
            throw new NullPointerException("False lock type");
        }
        return compatibleMatrix[indexA][indexB];
    }
    ...
    public static boolean canBeParentLock(LockType parentLockType, LockType childLockType) {
        ...
        if (childLockType == SIX){
            return parentLockType == IX;
        }
        return false;
    }

    private static int lockIndexHelper(LockType lockType){
        if (lockType == IS){
            return 0;
        }else if (lockType == IX){
            return 1;
    ...
    public static boolean substitutable(LockType substitute, LockType required) {
        ...
        // TODO(proj4_part1): implement
        if (required == NL || required.equals(substitute)){
            return true;
        }
