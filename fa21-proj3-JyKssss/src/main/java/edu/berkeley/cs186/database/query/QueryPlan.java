...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int minCost = minOp.estimateIOCost(), curIndex = -1;

        List<Integer> indexList = getEligibleIndexColumns(table);
        for (Integer index : indexList) {
            SelectPredicate curSelectPred = selectPredicates.get(index);
            QueryOperator indexOp = new IndexScanOperator(this.transaction,curSelectPred.tableName, curSelectPred.column, curSelectPred.operator, curSelectPred.value);
            int curCost = indexOp.estimateIOCost();
        ...
        minOp = addEligibleSelections(minOp, curIndex);
        return minOp;
    }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        List<JoinPredicate> curJoinPred = this.joinPredicates;
        for (Map.Entry<Set<String>, QueryOperator> prevQueryOperatorE : prevMap.entrySet()) {
            Set<String> tableSet = prevQueryOperatorE.getKey();
            QueryOperator prevOp = prevQueryOperatorE.getValue();

            for (JoinPredicate joinPredicate : curJoinPred) {
                String leftTable = joinPredicate.leftTable, leftColumn = joinPredicate.leftColumn,
                rightTable = joinPredicate.rightTable, rightColumn = joinPredicate.rightColumn;
                Set<String> addTableSet;
                int cost;
                QueryOperator bestJoinOp = null;
                if (tableSet.contains(leftTable) && !tableSet.contains(rightTable)){
                    ...
                    bestJoinOp = minCostJoinType(prevOp, rightOp, leftColumn, rightColumn);
                    cost = bestJoinOp.estimateIOCost();
                    addTableSet = new HashSet<>(tableSet);
                    addTableSet.add(rightTable);
                    QueryOperator oldOp = result.getOrDefault(addTableSet, null);
                    if (oldOp == null || cost < oldOp.estimateIOCost()){
                        result.put(addTableSet, bestJoinOp);
                    }
                }
                    ...
                    bestJoinOp = minCostJoinType(prevOp, leftOp, rightColumn, leftColumn);
                    cost = bestJoinOp.estimateIOCost();
                    addTableSet = new HashSet<>(tableSet);
                    addTableSet.add(leftTable);
                    QueryOperator oldOp = result.getOrDefault(addTableSet, null);
                    if (oldOp == null || cost < oldOp.estimateIOCost()){
                        result.put(addTableSet, bestJoinOp);
                    }
    ...
    public Iterator<Record> execute() {
        ...
        Map<Set<String>, QueryOperator> pass0Map = new HashMap<>();
        for (String tableName : tableNames) {
            Set<String> singleTable = new HashSet<>();
            singleTable.add(tableName);
            pass0Map.put(singleTable,minCostSingleAccess(tableName));
        }

        if (tableNames.size() == 1){
            Set<String> singleTable = new HashSet<>();
            singleTable.add(tableNames.get(0));
            this.finalOperator = pass0Map.get(singleTable);
            addGroupBy();
            addProject();
            addSort();
            addLimit();
        ...
        Map<Set<String>, QueryOperator> passiMap = minCostJoins(pass0Map, pass0Map);
        while (passiMap.size() > 1){
            passiMap = minCostJoins(passiMap, pass0Map);
        }
        this.finalOperator = minCostOperator(passiMap);
        addGroupBy();
        addProject();
        addSort();
        addLimit();
