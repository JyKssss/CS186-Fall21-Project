...
public class SortOperator extends QueryOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        Run mergeList = makeRun();
        List<Iterator<Record>> iteratorList = new ArrayList<>();
        for (int i = 0; i < size; i++) {
            iteratorList.add(runs.get(i).iterator());
        }
        for (int i = 0; i < size; i++) {
            Iterator<Record>iterator = iteratorList.get(i);
            if (iterator.hasNext()){
                heap.offer(new Pair<>(iterator.next(), i));
            }
        }

        while (heap.size()>0){
            Pair<Record, Integer>smallestRecord = heap.poll();
            mergeList.add(smallestRecord.getFirst());
            if (iteratorList.get(smallestRecord.getSecond()).hasNext()){
                heap.offer(new Pair<>(iteratorList.get(smallestRecord.getSecond()).next(), smallestRecord.getSecond()));
            }
        }
        return mergeList;
    }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        int size = numBuffers - 1, index = 0;
        while (index < runs.size()){
            List<Run> oneSortedRun = new ArrayList<>();
            int cnt = 0;
            while (index < runs.size() && cnt < size){
                oneSortedRun.add(runs.get(index));
                index += 1;
                cnt += 1;
            }
            mergeRuns.add(mergeSortedRuns(oneSortedRun));
        }
        return mergeRuns;
    }
    ...
    public Run sort() {
        ...
        while (sourceIterator.hasNext()){
            Iterator<Record> bufferIterator = getBlockIterator(sourceIterator, getSchema(), numBuffers);
            runsList.add(sortRun(bufferIterator));
        }

        while (runsList.size() > 1){
            runsList = mergePass(runsList);
        }
        return runsList.get(0); // TODO(proj3_part1): replace this!
    }
